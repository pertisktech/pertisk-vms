...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --yes) YES=1; shift ;;
    -h|--help)
      echo "Usage: sudo $0 --image out/pertisk-node.iso --disk /dev/sdX --yes"
      exit 0
      ;;
    *) die "unknown arg $1" ;;
  esac
...
[[ -n "$IMAGE" && -f "$IMAGE" ]] || die "pass --image path/to/pertisk-node.iso (or .raw)"
...
echo "flash: done. Boot $DISK (USB) on the target machine."
